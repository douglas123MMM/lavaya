...
export default function LaundryLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (profile?.role !== 'laundry' && profile?.role !== 'admin') {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <p style={{ color: '#718096', marginTop: 4 }}>Panel de Lavanderia</p>
        </div>
        <form onSubmit={handleLogin}>
          <input style={styles.input} type="email" placeholder="correo@lavaya.com" value={email} onChange={(e) => setEmail(e.target.value)} />
